        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-5">
            <div class="md:flex md:items-center md:justify-between">
                <div class="flex-1 min-w-0">
                    <h1 class="py-0.5 text-2xl font-semibold text-gray-900">{{ $t('Category') }}</h1>
                </div>
            </div>
        </div>
                    ...
                    <ul>
                        <template v-for="(category, index) in categoryList">
                            <li :class="{'border-t border-gray-200': index !== 0}">
                                <router-link
                                    :to="'/dashboard/admin/category/' + category.id + '/edit'"
                                    ...
                                    <div class="flex items-center px-4 py-4 sm:px-6">
                                        <div class="min-w-0 flex-1 flex items-center">
                                            <div class="min-w-0 flex-1 pr-4 lg:grid lg:grid-cols-2 lg:gap-4">
                                                <div class="flex">
                                                    <div class="inline-block align-middle text-sm font-medium leading-5 text-gray-900">{{ category.name }}</div>
                                                </div>
                                            </div>
                                        </div>
                                        <div>
...
export default {
    ...
    methods: {
        getCategory() {
            const self = this;
            self.loading = true;
            axios.get('api/dashboard/admin/categories').then(function (response) {
