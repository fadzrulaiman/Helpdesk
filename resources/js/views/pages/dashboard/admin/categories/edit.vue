            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-5">
                <div class="md:flex md:items-center md:justify-between">
                    <div class="flex-1 min-w-0">
                        <h1 class="py-0.5 text-2xl font-semibold text-gray-900">{{ $t('Edit Category') }}</h1>
                    </div>
                </div>
            </div>
                ...
                <div class="mt-6 shadow sm:rounded-lg">
                    <div class="bg-white md:grid md:grid-cols-3 md:gap-6 px-4 py-5 sm:p-6">
                        <div class="md:col-span-1">
                            <h3 class="text-lg font-medium leading-6 text-gray-900">{{ $t('Category details') }}</h3>
                            <p class="mt-1 text-sm leading-5 text-gray-500">
                                {{ $t('Category details and customization') }}.
                            </p>
                        </div>
...
export default {
    ...
    methods: {
        saveCategory() {
            const self = this;
            self.loading = true;
            axios.put('api/dashboard/admin/categories/' + self.$route.params.id, self.category).then(function () {
                self.loading = false;
                self.$notify({
        ...
        getCategory() {
            const self = this;
            self.loading = true;
            axios.get('api/dashboard/admin/categories/' + self.$route.params.id).then(function (response) {
